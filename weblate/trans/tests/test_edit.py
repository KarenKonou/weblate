...
class ZenViewTest(ViewTestCase):
    ...
    def set_zen_suggestions(self, *, enabled: bool) -> None:
        self.user.profile.suggestions_in_zen = enabled
        self.user.profile.save(update_fields=["suggestions_in_zen"])
    ...
    def test_zen_suggestions_hidden_when_disabled(self) -> None:
        self.add_zen_suggestion()
        self.set_zen_suggestions(enabled=False)
        response = self.client.get(reverse("zen", kwargs=self.kw_translation))
        self.assertContains(response, "Hello, world")
        self.assertNotContains(response, "row-suggestions-")
        self.assertNotContains(response, "Nazdar svete!")

    def test_zen_suggestions_hidden_anonymous(self) -> None:
        self.add_zen_suggestion()
        self.client.logout()
        response = self.client.get(reverse("zen", kwargs=self.kw_translation))
        self.assertContains(response, "Hello, world")
