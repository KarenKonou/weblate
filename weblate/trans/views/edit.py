...
def perform_suggestion_action(
    ...
    Returns the performed mode, or None when nothing was performed
    (errors are reported through the messages framework).
    """
    # Parse suggestion ID
...
def get_zen_unitdata(
    ...
    show_suggestions = zen_show_suggestions(request)
    units = unit_set.prefetch_full(suggestion_details=show_suggestions).get_ordered(
        search_result["ids"]
    )
...
def zen_show_suggestions(request: AuthenticatedHttpRequest) -> bool:
    """Whether the current user wants suggestions shown in the Zen editor."""
    return request.user.is_authenticated and request.user.profile.suggestions_in_zen
...
def render_zen_unit_response(
    ...
    *,
    ...
    The target and review state are only included with sync_target or when
    the target differs from previous_target_hash, the editor is otherwise
    left alone so that pending edits are not lost.
    """
    # Reload the unit: accepting a suggestion changes it through a different
    # instance and the cached suggestions and checks are stale either way.
...
def zen_suggestion(request: AuthenticatedHttpRequest, path):
    ...
    # Rejecting and voting normally leave the target alone, but voting
    # can automatically accept the suggestion.
